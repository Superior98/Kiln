...
function signSession(sessionId: string): string {
  return createHmac('sha256', billingSecret()).update(sessionId).digest('hex');
}
...
function readSessionId(req: Parameters<IRouter['get']>[1] extends never ? never : any): string | null {
...
async function ensureSessionSubscription(sessionId: string): Promise<void> {
  ...
  await db.insert(subscriptionsTable).values({ id: randomUUID(), userId: sessionId, plan: 'free', status: 'inactive' });
}
...
async function upsertSubscription(input: {
  ...
  if (existing.length) {
    await db.update(subscriptionsTable).set(values).where(eq(subscriptionsTable.id, existing[0].id));
  } else {
    await db.insert(subscriptionsTable).values({ id: randomUUID(), ...values });
  }
}

router.get('/billing/plans', (_req, res) => {
  res.json({ plans: Object.values(EMBER_PLANS) });
});

router.get('/billing/me', async (req, res) => {
  const sessionId = readSessionId(req);
  if (!sessionId) {
    res.json({ plan: 'free', model: EMBER_PLANS.free.model, dailyRequests: EMBER_PLANS.free.dailyRequests });
    return;
  }
...
router.post('/billing/checkout', async (req, res) => {
  ...
  if (plan !== 'premium' && plan !== 'premium_plus') {
    res.status(400).json({ error: 'A paid plan is required.' });
    return;
  }
  try {
    ...
    const body = new URLSearchParams({
      ...
      'subscription_data[metadata][kiln_plan]': plan,
      'subscription_data[metadata][kiln_session_id]': sessionId,
      'metadata[kiln_plan]': plan,
      'metadata[kiln_session_id]': sessionId,
    });
    ...
    res.status(503).json({ error: error instanceof Error ? error.message : 'Unable to start checkout.' });
  }
});
...
router.post('/billing/portal', async (req, res) => {
  const sessionId = readSessionId(req);
  if (!sessionId) {
    res.status(401).json({ error: 'No billing session found.' });
    return;
  }
  try {
    const rows = await db.select().from(subscriptionsTable).where(and(eq(subscriptionsTable.userId, sessionId))).limit(1);
    const customerId = rows[0]?.stripeCustomerId;
    if (!customerId) {
      res.status(400).json({ error: 'No Stripe customer is attached to this billing session.' });
      return;
    }
    ...
    res.status(503).json({ error: error instanceof Error ? error.message : 'Unable to open billing portal.' });
  }
});
...
router.post('/billing/webhook', async (req, res) => {
  ...
  if (!Number.isFinite(timestamp) || Math.abs(Date.now() / 1000 - timestamp) > 300) {
    res.status(400).json({ error: 'Expired Stripe webhook.' });
    return;
  }
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed' && sessionId) {
      await upsertSubscription({ userId: sessionId, customerId, subscriptionId, plan, status: 'active', currentPeriodEnd: periodEnd });
    } else if (event.type === 'customer.subscription.created' || event.type === 'customer.subscription.updated') {
      const subMetadata = metadata;
      const subSessionId = typeof subMetadata.kiln_session_id === 'string' ? subMetadata.kiln_session_id : null;
      if (subSessionId) await upsertSubscription({ userId: subSessionId, customerId, subscriptionId, plan, status, currentPeriodEnd: periodEnd });
    } else if (event.type === 'customer.subscription.deleted') {
      const subMetadata = metadata;
      const subSessionId = typeof subMetadata.kiln_session_id === 'string' ? subMetadata.kiln_session_id : null;
      if (subSessionId) await upsertSubscription({ userId: subSessionId, customerId, subscriptionId, plan: 'free', status: 'canceled', currentPeriodEnd: periodEnd });
    }
